...
def parse_select_option(select_str: str) -> tuple[str, List[str]]:
    ...
    if "=" not in select_str:
        raise ValueError(
            f"Invalid --select format: '{select_str}'. "
            f"Expected format: collection=id1,id2,id3"
        )
    ...
    try:
        ...
        if select:
            ...
            try:
                ...
                replication_config.defaults["include_patterns"] = [f"^{coll}$" for coll in collections_to_replicate]
                
                print_info(f"Cascade mode: {len(collections_to_replicate)} collections to replicate")
                if cascade_result.skipped_collections:
                    print_warning(f"Skipped collections (0 docs): {', '.join(sorted(cascade_result.skipped_collections))}")
        ...
        # Filter by --collections option if provided
            ...
            replication_config.defaults["replicate_all"] = False
            replication_config.defaults["include_patterns"] = [f"^{coll}$" for coll in collection_list]
            ...
            replication_config.defaults["replicate_all"] = False
            replication_config.defaults["include_patterns"] = [f"^{coll}$" for coll in selected_collections]
        ...
        def create_progress_display():
            ...
            if select:
                ...
                started_collections = set(replicating_collections.keys()) | \
                ...
                if completed_collections:
                    table.add_row("", f"[bold green]✓ Completed ({len(completed_collections)})[/bold green]", "")
                    ...
                    if len(completed_collections) > 5:
                        table.add_row("", f"  [dim]... and {len(completed_collections) - 5} more[/dim]", "")
                    table.add_row("", "", "")  # Spacer
                
                # Failed section
                if failed_collections:
                    table.add_row("", f"[bold red]✗ Failed ({len(failed_collections)})[/bold red]", "")
                    for coll_name, error in failed_collections:
        ...
        with Live(create_progress_display(), console=console, refresh_per_second=2) as live:
            ...
            def on_progress(collection_name: str, status: str, result: Optional[ReplicationResult]):
                ...
                if status == "started":
                    ...
                    total_collections = len(replicating_collections) + len(completed_collections) + len(failed_collections)
