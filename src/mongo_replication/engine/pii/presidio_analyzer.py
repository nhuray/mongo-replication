...
class PresidioAnalyzer:
    ...
    def get_analyzer(self) -> AnalyzerEngine:
        ...
        if self._analyzer_engine is None:
            ...
            self._analyzer_engine = AnalyzerEngine(nlp_engine=nlp_engine, supported_languages=self._supported_languages)
    ...
    def _filter_allowlist(self, flattened: Dict[str, Any], allowlist_patterns: List[str]) -> Dict[str, Any]:
        """
        ...
        for field_path, value in flattened.items():
            ...
            for pattern in allowlist_patterns:
                if self._matches_pattern(field_path, pattern):
                    should_skip = True
                    logger.debug(f"Skipping field '{field_path}' (matches allowlist pattern '{pattern}')")
                    break
