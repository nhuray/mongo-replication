...
class OrchestrationResult:
    ...
    def successful_collections(self) -> List[str]:
        ...
        return [
            name for name, result in self.collection_results.items()
    ...
    def log_summary(self) -> None:
        ...
        logger.info("=" * 60)
        logger.info(f"✅ Successfully replicated: {len(self.successful_collections)}/{self.total_collections_processed} collections")
        ...
        logger.info(f"   Auto-discovered collections: {len(self.discovery.auto_discovered_collections)}")
...
class ReplicationOrchestrator:
    ...
    def _replicate_single_collection(
        ...
        try:
            ...
            logger.info(f"🔍 Orchestrator - Replicating {collection_name} with match_filter: {config.match}")
        ...
        try:
            ...
            for coll_name in discovery_result.included_collections:
                ...
                if coll_name in discovery_result.auto_discovered_collections:
                    ...
                    logger.info(f"   ⚙️  {coll_name}: Configured (PII: {len(config.pii_fields)} fields)")
            ...
            collections_succeeded = sum(1 for r in collection_results.values() if r.status == "completed")
            ...
            failed_collections = [
                name for name, result in collection_results.items()
                if result.status == "failed"
            ]
