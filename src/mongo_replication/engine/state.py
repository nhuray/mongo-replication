...
class StateManager:
    ...
    # ============================================================
    # These methods maintain compatibility with old code that uses
    # the previous API. They should be phased out over time.

    def create_load_id(self) -> ObjectId:
        """Create a new load ID (legacy compatibility).
        
        Deprecated: Use create_run() instead.
        
        Returns:
            ObjectId for the new run
        """
        logger.warning("create_load_id() is deprecated, use create_run() instead")
        return self.create_run()

    def mark_started(self, collection_name: str) -> None:
        """Mark a collection as started (legacy compatibility).
        
        Deprecated: Use start_collection() instead.
        This creates a dummy run for backward compatibility.
        
        Args:
            collection_name: Name of the collection
        """
        logger.warning("mark_started() is deprecated, use start_collection() instead")
        # Create a dummy run for legacy code
        run_id = ObjectId()
        self.start_collection(run_id, collection_name)

    def mark_completed(self, collection_name: str) -> None:
        """Mark a collection as completed (legacy compatibility).
        
        Deprecated: Use complete_collection() instead.
        
        Args:
            collection_name: Name of the collection
        """
        logger.warning("mark_completed() is deprecated, use complete_collection() instead")
        # Find the most recent running state for this collection
        state = self.state_collection.find_one(
            {"collection": collection_name, "status": "running"},
            sort=[("startedAt", -1)]
        )
        if state:
            self.complete_collection(
                state_id=state["_id"],
                documents_processed=state["documents"]["processed"],
                documents_succeeded=state["documents"]["succeeded"],
                documents_failed=state["documents"]["failed"],
            )

    def mark_failed(self, collection_name: str, error_message: str) -> None:
        """Mark a collection as failed (legacy compatibility).
        
        Deprecated: Use fail_collection() instead.
        
        Args:
            collection_name: Name of the collection
            error_message: Error description
        """
        logger.warning("mark_failed() is deprecated, use fail_collection() instead")
        # Find the most recent running state for this collection
        state = self.state_collection.find_one(
            {"collection": collection_name, "status": "running"},
            sort=[("startedAt", -1)]
