...
class JobManager:
    ...
    def discover_jobs() -> List[JobConfig]:
        ...
        if not jobs:
            raise ValueError(
                "No enabled jobs found. Set REP_<JOB>_ENABLED=true to enable a job."
            )
    ...
    def get_job(job_id: str) -> JobConfig:
        ...
        if not enabled:
            raise ValueError(
                f"Job '{job_id}' is not enabled. "
                f"Set {enabled_key}=true to enable this job."
            )
        ...
        if not source_uri:
            raise ValueError(
                f"Job '{job_id}' is missing source URI. "
                f"Set {source_uri_key}=mongodb://..."
            )
